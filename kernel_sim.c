...
/* ====== Sinais ====== */
static void on_irq0(int s)
{
    (void)s;
    got_irq0 = 1;
}
static void on_irq1(int s)
{
    (void)s;
    got_irq1 = 1;
}
...
static void dispatch_next()
{
    ...
    if (!rq_pop(&nx))
    {
        ...
        time_t now = time(NULL);
        if (now != last_print)
        { // evita flood quando fila está vazia
            last_print = now;
            log_ts_prefix();
...
{
    return (finished_count == nprocs) && (rq_count == 0) && (current == -1) && (io_busy == 0) && (io_count == 0);
}
...
static void schedule_loop()
{
    for (;;)
    {
        ...
        if (got_irq0)
        {
            ...
            {
                // Há pelo menos mais alguém pronto: preempta normalmente
                log_ts_prefix();
        ...
        if (all_done())
        {
            ...
            if (ic_pid > 0)
                waitpid(ic_pid, NULL, 0); // aguarda IC
            break;                        // sai do scheduler
        }
...
int main(int argc, char **argv)
{
    ...
    sigaction(SIGUSR1, &sa, NULL); // IRQ0
    sa.sa_handler = on_irq1;
    sigaction(SIGUSR2, &sa, NULL); // IRQ1
    sa.sa_handler = on_sysc;
    sigaction(SIGALRM, &sa, NULL); // “acorda kernel”
    signal(SIGCHLD, (void (*)(int))on_child_exit);

    /* Fork apps A1..An (com 1s de defasagem para visual) */
    log_ts_prefix();
